      ...
      mapChildren: (children) => {
        ...
        for (let i = 0; i < elements.length; i++) {
          ...
          if (Object.hasOwn(el.props, 'x-else-if')) {
            if (state === 'idle') {
              throw new Error(
                '"deepIfElse: x-else-if" found on an element, but the previous element has no "x-if" or "x-else-if"'
              )
            } else {
...
export const DeepIfElse = deepIfElse()(function DeepIfElse(props) {
