...
export const $deephocInfo = Symbol('deephoc-info')
...
function applyHocToVdom(opts: DeepHocOptions) {
  return function (
    ...
    let deephocInfo = element?.type?.[$deephocInfo]
    while (deephocInfo) {
      if (deephocInfo.opts.id === opts.id) {
        return element
      }
      deephocInfo = deephocInfo.wrappedComponent[$deephocInfo]
    }
 ...
 * @param {DeepHocOptions} opts - The options for the HOC
 * @returns {ComponentType} The wrapped component
 */
export function deepHoc<FC extends ComponentType>(
  wrappedComponent: FC,
  opts: DeepHocOptions
): FC {
  ...
  let hoc: any = function DeepHoc(props, ref) {
  ...
  while (baseCmp[$deephocInfo]) {
    hocsDisplayName = `${baseCmp[$deephocInfo].opts.id} < ${hocsDisplayName}`
    baseCmp = baseCmp[$deephocInfo].wrappedComponent
  }
  ...
  // function, so it couldn't look up the [$deephocInfo] record to get the
  // baseName.
  hocRender[$deephocInfo] = info

  cache.set(wrappedComponent, info)

  // Fast refresh (HMR) support. It needs the react-deephoc/plugin/vite to be
  // installed.
  if (import.meta.hot && window.__DEEPHOC_GLOBAL__) {
    // setTimeout is needed when the component imports come earlier than the
    // deephoc import. E.g.: import App from "./App"; import { DeepHocs } from
    // "react-deephoc"; In this case, control gets here before the first
    ...
    setTimeout(() => {
      ...
      if (!deepHocGlobal.cmpToIds) {
        console.error(
          "Deephoc: Can't find the component/id map. Did you forget to install the react-deephoc/plugin/vite plugin?"
        )
        return
      }

      // Get the id of the original base component
      const ids = deepHocGlobal.cmpToIds.get(baseCmp)
      ...
      if (ids) {
        ...
        // stack
        if (!deepHocGlobal.isReapplication) {
        ...
        deepHocGlobal.originalRegister(hoc, id + ' ' + hocsDisplayName)
      }
    })
  }
...
/**
 * Use it like this:
 *   <DeepHocs hocs={[deepClassFor, deepLoader, deepTryCatch]}>
 *     <App />
 *   </DeepHocs>
 */
export function DeepHocs(props: { children: ReactNode; hocs: Function[] }) {
...
if (import.meta.hot && window.__DEEPHOC_GLOBAL__) {
  Object.assign(window.__DEEPHOC_GLOBAL__, { deepHoc })
}
